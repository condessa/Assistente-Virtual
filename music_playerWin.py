                ...
                if 'entries' in info:
                    for entry in info['entries']:
                        if entry:
                            ...
                            canal = entry.get('channel', entry.get('uploader', 'Desconhecido'))
        ...
        def _processar():
            try:
                ...
                if melhor_resultado:
                    ...
                    # Não encontrou correspondência exata, mostrar diálogo
                    print(f"⚠️ Múltiplas versões encontradas, a mostrar diálogo")
                    if self.gui:
